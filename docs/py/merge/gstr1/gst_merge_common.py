...
_GSTIN_RE = re.compile(r"^\d{2}[A-Za-z]{5}\d{4}[A-Za-z][0-9A-Za-z]Z[0-9A-Za-z]$")
_DATE_RE = re.compile(r"^\d{1,2}[-/]\d{1,2}[-/]\d{2,4}$")


def _cell_looks_like_data(v):
    """Would a human recognise this cell as an actual GST record - a GSTIN,
    a date, an amount - rather than a column caption or section title?"""
    if v is None:
        return False
    ...
    if isinstance(v, (datetime.date, datetime.datetime)):
        return True
    s = str(v).strip()
    return bool(_GSTIN_RE.match(s) or _DATE_RE.match(s))


def detect_header_rows(ws, max_scan_row=25, min_data_row=2):
    """How many rows at the top of this sheet are header, not data.

    Two portal template families have shown up in practice: GSTR-2B, where
    the deepest header row uses merged cells to align grouped sub-columns
    (so 'the highest row touched by a merge' finds the header boundary);
    and GSTR-1, where the deepest header row is a single flat row of plain
    column captions with NO merges at all (so that signal doesn't exist).

    Rather than maintain two different detectors, this looks for what
    actually marks the header/data boundary regardless of layout: the
    first row containing a cell that looks like real GST data (a GSTIN, a
    date, or a number) rather than a label. That's true in both template
    families and isn't tied to any particular row count, so it keeps
    working if a future template adds or removes a header row.

    Falls back to (widest top merge's last row) + 1 for a sheet with zero
    data rows in this file, where the data-boundary signal doesn't exist -
    the exact number matters less there since there's nothing to
    misalign, but it keeps the copied header block sensible.
    """
    n_cols = ws.max_column
    for r in range(min_data_row, min(max_scan_row, ws.max_row) + 1):
        for c in range(1, n_cols + 1):
            if _cell_looks_like_data(ws.cell(row=r, column=c).value):
                return r - 1

    wide_merges = [
        rng.max_row for rng in ws.merged_cells.ranges
        if rng.min_row <= max_scan_row and (rng.max_col - rng.min_col + 1) >= max(1, n_cols * 0.5)
    ]
    return (max(wide_merges) + 1) if wide_merges else 4
