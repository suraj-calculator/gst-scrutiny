...
def load_data_workbook(path):
    """Load a workbook for READING ACTUAL DATA (row contents) - never for
    just checking sheet names/type (use detect_file_type's own read_only
    load for that, it's cheap and safe since it never touches row data).

    Always loads in normal (non-read-only) mode, deliberately.

    Why this matters: GSTN-portal-generated workbooks (confirmed directly
    against a real R2A file) can carry a stale/incorrect <dimension> tag in
    their XML. openpyxl's read_only=True mode trusts that tag blindly for
    max_row - on the file in question it reported max_row=30 when the true
    last data row was 3403, which would silently drop ~99% of that sheet's
    rows with no error and no warning. Normal (non-read-only) mode parses
    every cell and is not vulnerable to this.

    Every merge_*.py script's data-reading load_workbook() call should go
    through this function rather than calling openpyxl directly, so a
    future edit can't reintroduce read_only=True for a data read by
    accident.
    """
...
def detect_file_type(path):
    """Return 'EINV', 'GSTR1', 'GSTR3B', 'GSTR2B', 'R2A', or None based on sheet-name signature."""
    try:
    ...
    if "Read me" in names and "b2b, sez, de" in names:
        return "EINV"
    if "Read me" in names and "ECO" in names and "TDS" in names:
        return "R2A"
    return None
